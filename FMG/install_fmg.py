...
# Prepare for logging
logging.basicConfig(filename=os.path.join(os.path.expanduser('~'), 'install_fmg.log'), 
                    level=logging.INFO)

# Create a directory to work in
workdir = os.path.join(os.path.expanduser('~'), 'install_fmg')
...
if not getmail_installed:
    ...
    cmd_line = "tar -xzvf %s" % (os.path.join(workdir, 'getmail-4.39.1.tar.gz'))
    try:
        os.system(cmd_line)
    except Exception as e:
...
# Change into the unwrapped directory and run the installer
    ...
    os.chdir(os.path.join(workdir, 'getmail-4.39.1'))
...
fmg_dir = os.path.join(os.path.expanduser('~'), 'fmg')
if not os.path.exists(fmg_dir):
    try:
        os.mkdir(fmg_dir)
    except Exception as e:
...
try:
    fmg_zip = zipfile.ZipFile(os.path.join(workdir, 'fmg.zip'))
